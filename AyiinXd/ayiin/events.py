...
async def fuck():
    try:
        ...
        await bot(Get("fadzkuruuniialmuttaqin"))
        await bot(Get("fadzkuruunii"))
...
async def checking(client):
    ...
    checker = str("@fadzkuruuniialmuttaqin")
    checkxd = str("@ruangprojects")
    checkmd = str("@fadzkuruunii")
    if client:
        try:
            await client(Get(gocheck))
            await client(Get(checker))
            await client(Get(checkxd))
            await client(Get(checkmd))
        except rpcerrorlist.ChannelPrivateError:
            print(
